...
class UserModelViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save(password= bcrypt.hashpw(self.request.data['password'].encode('utf-8'),bcrypt.gensalt()).decode('utf-8'),token = uuid4())
            res = dict(serializer.data)
            print(res.pop('accountID'))
            res.pop('password')
...
def login(request):
    ...
    if not user or not bcrypt.checkpw(password.encode('utf-8'), bytes(list(user.values())[0]['password'], 'utf-8')):
        return Response({"error":"Username hoặc password không đúng."},status=status.HTTP_400_BAD_REQUEST)
    else :
        user.update(token = uuid4())
        user = list(user.values())[0]
        user.pop('accountID')
        user.pop('password')
        return Response(user,status = status.HTTP_200_OK)
